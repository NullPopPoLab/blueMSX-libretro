...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
bool am3u_setup_c(AdvancedM3U* inst,const char* src,const char* basedir,AdvancedM3UDeviceSelector cbsel,AdvancedM3UParseError cberr,void* user){
...
bool am3u_device_remove_media(AdvancedM3UDevice* inst,int slot){
