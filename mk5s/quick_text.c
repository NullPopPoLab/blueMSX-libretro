...
bool qtext_is_empty(const QTextRef* src){
	if(!src)return true;
	return src->len<1;
}
//! get length of a QTextRef 
size_t qtext_len(const QTextRef* src){
	if(!src)return 0;
	return src->len;
}
//! get beginside of a QTextRef 
const char* qtext_bgn(const QTextRef* src){
	if(!src)return NULL;
	return src->bgn;
}
//! get endside of a QTextRef 
const char* qtext_end(const QTextRef* src){
	if(!src)return NULL;
...
*/
char* qtext_alloc_q(const QTextRef* src){
	if(!src)return NULL;
...
/*!	@param src  source text
	@return  pointer of copied standard C string
*/
char* qtext_alloc_c(const char* src){
	if(!src)return NULL;
...
*/
const char* qtext_ext_lines_c(void* user,const char* src,QTextExtracted cbext){
	QTextRef src_q;
...
void qtext_trim_both_q(QTextRef* target,const QTextRef* letters){
	qtext_trim_fore_q(target,letters);
	qtext_trim_back_q(target,letters);
}

void qtext_trim_fore_c(QTextRef* target,const char* letters){
	QTextRef letters_q;
	qtext_ref_c(&letters_q,letters);
	qtext_trim_fore_q(target,&letters_q);
}

void qtext_trim_back_c(QTextRef* target,const char* letters){
	QTextRef letters_q;
	qtext_ref_c(&letters_q,letters);
	qtext_trim_back_q(target,&letters_q);
}

void qtext_trim_both_c(QTextRef* target,const char* letters){
	QTextRef letters_q;
	qtext_ref_c(&letters_q,letters);
...
void qtext_trim_fore_q(QTextRef* target,const QTextRef* letters){
	...
	while(1){
		bool done=false;
		const char* c;
		for(c=qtext_bgn(letters);target->len>0 && c<qtext_end(letters);++c){
